...
function bienvenida() {
    ...
    if (usuario === undefined) {
        ...
        window.alert(
            `Bienvenido a Skylab Airlines ${usuario}! A continuación verás toda la información de nuestros vuelos.`
        );
    }
}
...
function mostrarVuelos() {
    ...
    for (let i = 0; i < flights.length; i++) {
        if (flights[i]["scale"] === true) {
            console.log(
                `El vuelo con ID ${flights[i].id} con origen ${flights[i].from} y destino ${flights[i].to} tiene un coste de ${flights[i].cost}€ y realiza escala.`
            );
        } else {
            console.log(
                `El vuelo con ID ${flights[i].id} con origen ${flights[i].from} y destino ${flights[i].to} tiene un coste de ${flights[i].cost}€ y no realiza escala.`
            );
        }
    }
}
...
function crearEliminar() {
    // Si quiere añadir o eliminar vuelos
    let crearElimVuel;
...
function crearVuelos() {
    if (flights.length > 15) {
        window.alert(
            "Lo sentimos, solo se permite introducir un máximo de 15 vuelos, puede eliminar alguno para poder continuar."
        );
        eliminar();
        return;
    } else {
        ...
        let masVuelos = window.prompt(
            "¿Desea añadir otro vuelo? Escribe Sí/ No"
        );
        if (
            flights.length < 15 &&
            (masVuelos === "sí" ||
...
function eliminar() {
    ...
    if (eliminarVuelo === false) {
        ...
        let vueloId = parseInt(
            window.prompt(
                "Introduzca el ID del vuelo que desea eliminar",
                "Por ejemplo: 07"
            )
        );
        if (vueloId === null || vueloId === "" || isNaN(vueloId)) {
            ...
            for (i = 0; i < flights.length; i++) {
                ...
                console.log(
                    `La actual lista de vuelos es:\n El vuelo ID ${
                        flights[i].id
                    } con origen ${flights[i].from} y destino ${
                        flights[i].to
                    } tiene un coste de ${flights[i].cost}€ y ${
                        flights[i].scale === true
                    }? 'realiza escala': 'no realiza ninguna escala'.`
                );
            }
        }
    }
...
function buscarPrecio() {
    ...
    let ordenar = window.prompt(
        "Indica como deseas ver el orden de precios: \nMás alto, introduce: alto \nMás bajo, introduce: bajo \nIgual, introduce: igual"
    ).toLowerCase;
...
function comprarVuelo() {
    let indice = parseInt(
        prompt(
            "Para comprar el vuelo introduzca el ID, en caso contrario pulse Cancelar"
        )
    );
