...
function calculadoraPro() {
    ...
    do { 
        newNumber= prompt('Introduzca un número o presiona cancel para detener');
        if(!isNaN(newNumber)&&(newNumber!="")&&(newNumber!= null)){
...
if (numberList.length === 1){ //Si es un solo numero mostramos solo la raíz cuadrada
    window.alert(`La raíz cuadrada de ${numberList[0]} es ${raiz}.`);
    
} else if (numberList.length > 1){ //Si es más de 1 hacemos suma, resta, multiplicación y división
...
if(numberList.length>1){
    window.alert(`El resultado de la suma es ${resultSum.toFixed(3)} \n El resultado de la resta es ${resultRest.toFixed(3)} \n El resultado de la multiplicación es ${resultMult.toFixed(3)} \n El resultado de la división es ${resultDiv.toFixed(3)}`); 
}
